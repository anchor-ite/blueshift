...
#[program]
pub mod anchor_flashloan {
    use super::*;

    pub fn borrow(ctx: Context<Loan>, borrow_amount: u64) -> Result<()> {
        //Make sure we are not sending in an invalid amount that can crash our Protocol
        ...
        //Transfer the fund from the protocol to the borrower
        transfer(
...
if let Ok(repay_ix) = load_instruction_at_checked(len as usize - 1, &ixs) {
    ...
    require_keys_eq!(repay_ix.accounts.get(3).ok_or(
        ProtocolError::InvalidBorrowerAta)?.pubkey, 
        ctx.accounts.borrower_ata.key(), 
        ProtocolError::InvalidBorrowerAta);
    require_keys_eq!(repay_ix.accounts.get(4).ok_or(
        ProtocolError::InvalidProtocolAta)?.pubkey, 
